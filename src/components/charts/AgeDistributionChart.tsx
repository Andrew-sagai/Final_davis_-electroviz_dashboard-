...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, color: '#8b5cf6' }}>Count: {payload[0].value.toLocaleString()}</p>
    </div>
  );
};
...
export default function AgeDistributionChart({ data, delay = 0 }: AgeDistributionChartProps) {
  return (
    <GlassCard title="Distribusi Usia" subtitle="Rincian kelompok usia pelanggan" icon={Users} delay={delay}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 25 }}>
            ...
            <XAxis dataKey="range" tick={{ fill: '#FF8599', fontSize: 11 }} axisLine={{ stroke: 'rgba(255, 42, 75, 0.2)' }} />
            ...
            <Bar dataKey="count" radius={[6, 6, 0, 0]} maxBarSize={45} animationDuration={1200}>
              {data.map((_, i) => (
